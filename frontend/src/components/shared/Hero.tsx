    ...
    <section className="relative flex min-h-[72vh] flex-col items-center justify-center px-4 pt-24 pb-14">
      ...
      >
        <div className="space-y-5">
          <Badge variant="outline" className="border-(--gmp-line-strong) bg-(--gmp-bg-panel) px-3 py-1 font-mono text-[11px] tracking-[0.18em] text-(--gmp-accent) uppercase">
            <Compass className="size-3.5" />
            Industrial Story Layer
          </Badge>

          <h1 className="max-w-4xl font-heading text-4xl leading-[1.05] font-semibold tracking-tight text-(--gmp-text-primary) md:text-6xl lg:text-7xl">
            简洁结构，
            <span className="mt-2 block text-(--gmp-accent)">高密度信息表达。</span>
          </h1>

          <p className="max-w-2xl text-base leading-relaxed text-(--gmp-text-secondary) md:text-lg">
        ...
        <div className="flex w-full flex-col gap-6 md:flex-row md:items-end md:justify-between">
          <div className="flex flex-col gap-3 sm:flex-row">
            <Button asChild size="lg" className="h-11 min-w-44 border border-(--gmp-line-strong) bg-(--gmp-accent) text-black hover:bg-[#e2d3b4]">
              <Link href="/search">
                查看内容流
                <ArrowRight className="size-4" />
              </Link>
            </Button>
            <Button asChild variant="outline" size="lg" className="h-11 min-w-44 border-(--gmp-line-strong) bg-(--gmp-bg-panel) text-(--gmp-text-primary) hover:bg-(--gmp-bg-elevated)">
              <Link href="/register">
          ...
          <div className="grid w-full max-w-xl grid-cols-1 gap-2 border border-(--gmp-line-soft) bg-(--gmp-bg-panel) p-3 sm:grid-cols-3 gmp-cut-corner-br gmp-hard-shadow">
            {heroMetrics.map((item) => (
              <div key={item.label} className="rounded-xl border border-(--gmp-line-soft) bg-[rgba(255,255,255,0.02)] px-3 py-2.5">
                ...
                <p className="mt-1.5 inline-flex items-center gap-1.5 text-sm font-medium text-(--gmp-text-primary)">
                  <Gauge className="size-3.5 text-(--gmp-accent-dim)" />
                  {item.value}
                </p>
              </div>
            ))}
