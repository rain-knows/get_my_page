  ...
  return (
    ...
    >
      ...
      <div className="pointer-events-none absolute top-12 right-12 hidden font-mono text-[10px] font-bold tracking-[0.3em] text-white/40 uppercase lg:block">
        <div className="border-t-2 border-(--gmp-accent) w-8 mb-2" />
        INIT_SEQ // 0xFD42A
      </div>
      ...
      <div className="relative z-10 flex min-h-dvh flex-col justify-between px-8 py-10 md:px-16 md:py-16">
        ...
        <div className="flex items-start justify-between">
          <div className="gmp-cut-corner-br flex flex-col border-l-4 border-(--gmp-accent) bg-(--gmp-bg-panel) px-6 py-4">
            <span className="font-heading text-sm font-bold tracking-widest text-white uppercase mb-1">
              GLOBAL STAGE
            </span>
            <span className="font-mono text-[9px] tracking-[0.2em] text-(--gmp-text-secondary) uppercase">
        ...
        <div className="flex w-full flex-col items-center justify-center self-center pb-10 text-center">
          ...
          <div className="mt-4 flex w-full max-w-md items-center justify-center gap-4">
            <div className="h-0.5 flex-1 bg-(--gmp-line-strong)" />
          ...
          <div className="mt-16 w-full max-w-xl">
            <div className="mb-3 flex items-end justify-between font-mono font-bold uppercase tracking-widest text-[#FFF]">
              ...
              <span className="text-2xl leading-none text-(--gmp-accent)">{Math.round(clamped)}%</span>
            </div>

            {/* 锐利边框容器 */}
            <div className="h-3 w-full bg-(--gmp-bg-panel) gmp-cut-corner-l border border-(--gmp-line-strong) p-px">
              <motion.div
                className="h-full bg-(--gmp-accent) gmp-cut-corner-l"
        ...
        {/* 底部工业标识 */}
        <div className="flex items-end justify-between border-t-2 border-(--gmp-line-soft) pt-6">
          <span className="font-mono text-[9px] font-bold tracking-[0.2em] text-(--gmp-text-secondary) uppercase">
            © 2026 Endfield Technology
          </span>
          <div className="gmp-cut-corner-tl bg-(--gmp-bg-elevated) px-4 py-2 font-mono text-[9px] tracking-widest text-white/40 uppercase">
            STATUS: <span className="text-(--gmp-accent) font-bold">READY</span>
          </div>
        </div>
      </div>
    </motion.section>
