...
| 项目 | 命令 | 期望 | 实际 | 是否通过 |
|---|---|---|---|---|
| 成功路径 | curl -sS -X POST http://localhost:8080/api/auth/login -H "Content-Type: application/json" -d "{\"username\":\"admin\",\"password\":\"admin123\"}" | 返回 code=200 或可预期成功结构 | 待填写 | 待填写 |
| 失败路径 | curl -sS -X POST http://localhost:8080/api/auth/login -H "Content-Type: application/json" -d "{\"username\":\"admin\",\"password\":\"wrong\"}" | 返回明确错误码或失败状态 | 待填写 | 待填写 |
