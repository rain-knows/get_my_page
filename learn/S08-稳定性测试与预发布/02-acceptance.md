...
## 执行命令
- docker compose config >/tmp/docker-config.out && echo ok
- docker compose -f not-exists.yml config
...
| 项目 | 命令 | 期望 | 实际 | 是否通过 |
|---|---|---|---|---|
| 成功路径 | docker compose config >/tmp/docker-config.out && echo ok | 返回 code=200 或可预期成功结构 | 待填写 | 待填写 |
| 失败路径 | docker compose -f not-exists.yml config | 返回明确错误码或失败状态 | 待填写 | 待填写 |
