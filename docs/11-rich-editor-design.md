...
- 链接、视频、音乐统一走“导入链接卡片”解析器；删除 YouTube/Twitter 独立命令入口。
...
- 编辑器业务模块：`frontend/src/features/post/editor/novel-demo/*`
  ...
  - `embed-link-extension.tsx`：`embedLink` 节点与卡片 NodeView 渲染（链接/上传双模式 + 平台专用模板）。
...
- 链接卡片契约：
  ...
  - `music + netease`、`video + bilibili` 命中平台专用模板；其余走通用模板回退。
...
- 平台模板可用：网易云音乐、B站视频分别命中专用卡片样式；解析失败保留 URL 降级卡片，不阻断编辑。
