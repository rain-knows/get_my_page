...
- 编辑页首次进入以服务端 `GET /api/posts/{slug}` 数据为准，localStorage 草稿仅用于标题恢复与离线兜底。
- 编辑器通过"发布到服务器"按钮调用 `PUT /api/posts/{id}` 将编辑内容写回后端，发布成功后自动清理本地草稿。
- 新建文章通过 `/blog/new` 路由进入 create 模式编辑器，发布时调用 `POST /api/posts`。
- 编辑内容草稿存储为 `localStorage`（按 `slug` 隔离），500ms 防抖自动保存 json/html/markdown 三种格式。
...
- 阅读组件：`frontend/src/features/post/components/BlogArticleReader.tsx`
  - 默认读取后端文章详情。
  - 仅在显式 `?preview=local` 模式下读取同 slug 的 localStorage 草稿标题与正文。
...
- 编辑页行为（2026-05-05 更新）：
  - 编辑页首次进入以服务端数据为准（`GET /api/posts/{slug}`），localStorage 仅作离线兜底。
  - 编辑器提供"发布到服务器"按钮，调用 `PUT /api/posts/{id}` 将编辑内容持久化到后端。
  - 新建文章通过 `/blog/new` 路由 + `POST /api/posts` 创建。
  - 发布成功后自动清理对应 slug 的 localStorage 草稿。
  - 本地草稿不存在且服务端不可用时，回退到空默认文档而不是官方演示文档。
- 上传契约：
...
- 打开 `/blog/{slug}/edit`，首次无缓存时展示后端该文章的真实标题与正文。
...
- 阅读页与编辑页内容链路一致：未带 `?preview=local` 的阅读页、列表页、搜索页与编辑页首次加载均对应同一份后端文章数据。
