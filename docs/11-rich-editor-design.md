...
| 卡片节点扩展 | P1 | Slash 菜单可插入 GitHub/音乐/视频/链接卡片 |
| AI 写作能力 | 关闭 | 一期关闭，前端无 AI 入口 |
...
- 空卡片策略：支持插入“外链空卡片”和“代码空卡片”，未导入内容使用空卡片视觉态。
- 独立编辑界面：选中外链卡片或代码块后，在 `INSERT EDITOR` 面板单独编辑并应用。
...
- 当前实现：编辑器顶部显示 `SAVING... / SAVED / SAVE FAILED` 状态徽标，失败后可手动重试保存。
